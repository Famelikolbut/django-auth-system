...
def populate_initial_data(apps, schema_editor):
    ...
    perm_view_own_docs, _ = permission_model.objects.get_or_create(name='view_own_documents', defaults={'description': 'Разрешает пользователю просматривать свои документы.'})
    perm_view_reports, _ = permission_model.objects.get_or_create(name='view_financial_reports', defaults={'description': 'Разрешает просматривать финансовые отчеты.'})
    perm_manage_perms, _ = permission_model.objects.get_or_create(name='manage_permissions', defaults={'description': 'Разрешает управлять разрешениями.'})
    perm_manage_roles, _ = permission_model.objects.get_or_create(name='manage_roles', defaults={'description': 'Разрешает управлять ролями.'})
    perm_assign_roles, _ = permission_model.objects.get_or_create(name='assign_roles', defaults={'description': 'Разрешает назначать роли пользователям.'})
