...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if email and password:
            ...
            # Добавляем явную проверку, является ли полученный user экземпляром нашего CustomUser
            if not isinstance(user, CustomUser):
                # Эта ситуация не должна произойти в нашем проекте, но это "защита от дурака"
...
class RoleSerializer(serializers.ModelSerializer):
    ...
    # Используем вложенный сериализатор, чтобы видеть полную информацию о разрешениях, а не просто их ID.
    permissions = PermissionSerializer(many=True, read_only=True)
    # Это поле будет использоваться для назначения разрешений по их ID при создании/обновлении роли.
