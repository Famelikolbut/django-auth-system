...
class HasPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        # 3. Получаем список кодовых имен разрешений, которые требуются для доступа к этой View.
        ...
        # 6. Проверяем, есть ли у пользователя хотя бы одно из требуемых разрешений.
        #    Мы ищем пересечение множества разрешений пользователя и множества требуемых разрешений.
        if not user_permissions.intersection(required_permissions):
            # Если пересечения нет, значит, ни одного нужного права у пользователя нет.
            return False
